...
def process_issue_comment(webhook_body):
    ...
    pull_url = payload["repository"]["pulls_url"].replace(
        "{/number}", f"/{pull_number}"
    )
    ...
    if get_pull_response is not None:
        ...
        if relevant_org and relevant_comment:
            ...
            if not relevant_comment:
                logging.error(
                    f"The comment '{comment_body}' does not contain magic phrase {TRIGGER_PHRASE}"
                )
            else:
                logging.error(
                    f"The PR {pull_number} organization {pull_user} does not match {ORGANIZATION_NAME}"
                )
    else:
